...
SELECT COUNT( DISTINCT customer_id) AS toal_customer FROM retail_sales;
...
SELECT category, gender, COUNT(transactions_id) AS total_no_of_transaction 
FROM retail_sales
...
FROM (	SELECT EXTRACT(YEAR FROM sale_date) AS year, EXTRACT(MONTH FROM sale_date) AS month,AVG(total_sale) AS avg_sale,
...
--Q.9 Write a SQL query to find the number of unique customers who purchased from items from each category
